...
def check_bash(ti: dict) -> None:
    ...
    # A. 개인키 — **경로를 넘기는 것과 내용을 읽는 것을 구분한다.**
    #
    #    [사건] 첫 판은 "명령에 .pem이 있으면 deny, 단 chmod·ls·--help가 같이 있으면 통과"
    #    였다. 그래서 `ssh -i <키> ... nvidia-smi` 같은 **정당한 접속이 전부 막혔고**,
    #    반대로 명령 안에 `ls`가 우연히 들어가면 통과했다. **판정이 우연에 달려 있었다.**
    #    서버 작업을 전담하게 되면서 첫 명령부터 막혀 발견했다.
    #
    #    키의 위험은 읽기·복사·전송이지 `-i`로 경로를 넘기는 것이 아니다. 명령어로 가른다.
    #    [폐기] 서버 회수(2026-11-05) 이후 키가 사라지면.
    #    **명령 전체에서 위험 단어를 찾지 않는다.** 키 경로 **바로 앞에 무엇이 오는지**만 본다.
    #    [사건] 단어 검색 방식은 `scp -i <키> f.tar host:/` 를 막았다 — 파일명의 `tar`를
    #    "키를 압축하는 명령"으로 읽었다. 업로드 계획이 tar 스트리밍이라 그대로면
    #    데이터 전송 자체가 불가능했다.
    for m in SECRET_PAT.finditer(cmd):
        before = cmd[:m.start()]
        # 키를 **인자로 소비**하는 정당한 형태: -i / --identity / IdentityFile= 뒤,
        # 또는 권한·조회 명령의 인자.
        if re.search(r"(-i|-I|--identity(?:_file|file)?|IdentityFile=)\s*\S*$", before) or \
           re.search(r"\b(chmod|chown|stat|ls|file|test|\[)\s+(\S+\s+)*\S*$", before):
            continue
        decide("deny",
               "개인키를 읽거나 옮기려는 명령으로 보인다.\n"
               "허용: `ssh/scp/rsync -i <키>` 처럼 **경로만 인자로 넘기는** 사용, chmod·stat 등.\n"
               "금지: cat·cp·base64·tar·curl 등으로 키를 **읽거나 복사하거나 전송**하는 것.\n"
               "이 저장소는 public이고, 키가 한 번 새면 파일을 지워도 재발급해야 한다.")

    # A2. 되돌릴 수 없는 삭제 — **차단이 아니라 묻는다**(임시 파일 정리는 정당하다).
    #     [사건] 서버 작업을 전담하게 됐다. 업로드에 몇 시간 걸리는 27GB가 한 줄로 날아간다.
    #     서버 명령도 전부 이 Bash를 지나므로 여기서 걸린다.
    #     [폐기] 없음.
    if re.search(r"\brm\s+(-\w*\s+)*-\w*[rRf]|\brm\s+-[rRf]|\bshred\b|\bmkfs\b|"
                 r"\bdd\s+[^|;&]*of=|\btruncate\s+-s\s*0", cmd):
        decide("ask",
               "되돌릴 수 없는 삭제 명령이다.\n"
               "무엇을 지우는지, 그것이 유일본인지 먼저 확인한다. 이 프로젝트에서 유일본은 "
               "v11 체크포인트·processed_full(27GB)·results/이고, 서버 업로드는 몇 시간 걸린다.\n"
               "지울 경로와 지킬 경로의 관계를 먼저 적는다 — 한쪽이 다른 쪽의 조상이면 멈춘다.")
