    ...
    **왜 이 조건이 필요한가.** v11 체크포인트에서 게이트 값 g를 직접 재보니
    (`scripts/inspect_prosody_gate.py`, 96발화 x 512차원) 이렇게 나왔다:
      ...
      - 그런데 소음이 와도 **총량을 안 옮긴다.** SNR 10dB에서 방향은 유의하지만
        (72/96 발화, p=4.85e-07) 옮긴 양이 평소 변동의 4.7%다. 같은 조건에서
        정확도는 46.19% -> 37.18%로 무너지는데 g는 0.4895 -> 0.4788만 움직인다

    [낡은 수치] 위 소음 수치(37.18%)와 게이트 값은 **13.5절 이전 시드**로 잰 것이다.
    소음 증강을 붙이면서 잡음 시드를 (발화)에서 (발화, SNR)로 바꿨으므로(SNR마다 독립
    실현을 주려면 필요했다) 잡음 실현이 달라졌다. 재측정 대상이고, 결론("게이트가
    총량을 안 옮긴다")은 실현이 바뀌어도 유지될 것으로 본다 — 옮긴 양이 평소 변동의
    4.7%라 실현 차이로 뒤집힐 크기가 아니다.
