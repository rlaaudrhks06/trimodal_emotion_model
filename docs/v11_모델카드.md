...
배포 대상은 **Jetson Orin Nano**(2023)다. 구형 Jetson Nano(2019)와 세대가 다르다 —
메모리 8GB(vs 4GB), Ampere 1024코어 + Tensor 코어(vs Maxwell 128코어), 대역폭
약 68GB/s(vs 25.6GB/s).

| 항목 | 판정 |
|---|---|
| **메모리** | int8 0.42GB + 런타임·부대비용 약 1.7GB → **8GB 중 6GB가 강화학습 몫으로 남는다** |
| **속도** | 맥 대비 3~5배 추정 → fp32 1.0~1.8초, **TensorRT + INT8로 0.3~0.8초 기대** |
| **간섭** | 모터 제어는 밀리초 지연이 필요 — GPU 점유 스케줄링 설계 필요 |

**Tensor 코어가 INT8을 하드웨어 가속**하므로 양자화 효과가 크다. **백본을 통째로 갈아엎을
필요는 없어 보인다.**

필요한 작업: TensorRT 변환 → INT8 양자화 → 프레임 수 조정. 앞의 둘은 정확도를 거의
잃지 않는다.

**미확인**: ① 4GB 모델인지 8GB 모델인지 ② 강화학습 정책 크기와 제어 주기
③ **실제 보드에서의 실측값 — 위 수치는 전부 추정이다**
