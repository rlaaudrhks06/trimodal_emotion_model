...
![v11 데이터 흐름. 8초 발화·얼굴 24장·토큰 12개를 넣었을 때 각 단계의 실제 텐서 크기](assets/architecture_v11_flow.png)

> 위 두 그림은 `scripts/build_architecture_diagrams.py`로 생성한다. 앞은 **왜 그렇게 했는지**를,
> 뒤는 **텐서가 어떻게 흐르는지**를 보여준다. 뒤 그림의 형태는 실제 forward에 훅을 걸어
> 측정한 값이다.
>
> 비교용 도면: 처음 설계(이중모달)는 통합기록 2.2절, 직전 설계(v10)는
> `assets/architecture_v10.png` — **v10에서 바뀐 것은 오디오 백본 하나뿐이다.**
